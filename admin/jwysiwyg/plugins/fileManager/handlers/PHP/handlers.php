...
class RenameHandler extends ResponseHandler {
	public function getResponse ($router) {
		...
		if(rename($root.$dir.$file, $root.$dir.$router->cleanFile($_GET['newName']))) {
			return array(
				"success" => true,
				"data" => "Renombrado correctamente."
			);
		}
		else {
			return array(
				"success" => false,
				"error" => "No se ha podido renombrar el fichero."
			);
		}
	}
}
...
class RemoveHandler extends ResponseHandler {
	public function getResponse ($router) {
		...
		if ($is_removed) {
			return array(
				"success" => true,
				"data" => "Fichero borrado correctamente."
			);
		} else {
			return array(
				"success" => false,
				"error" => "No se ha podido borrar el fichero."
			);
		}
	}
}
...
class MoveHandler extends ResponseHandler {
	public function getResponse ($router) {
		...
		if(rename($root.$dir.$file, $root.$newPath)) {
			return array(
				"success" => true,
				"data" => "Fichero movido correctamente."
			);
		}
		else {
			return array(
				"success" => false,
				"error" => "No se ha podido mover el fichero."
			);
		}
	}
}
...
class UploadHandler extends ResponseHandler {
	public function getResponse ($router) {
		...
		if (file_exists($dst)) {
            ...
            return 'ERROR: El fichero ya existe.
