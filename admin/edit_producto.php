...
if( isset( $_POST['nombre'] ) ) {
    ...
    //$consulta="update jcms_productos set nombre='".$nombre."', descripcion='".$descripcion."', id_seccion='".$id_seccion."', precio='".$precio."', orden='".$orden."' where id='".$id."'";
    ...
    if(!$query = mysql_query($consulta)) { 
        ...
        if (isset($_POST["fotos"]) && is_array($_POST["fotos"]) && count($_POST["fotos"]) > 0) { 
            ...
            /*foreach ($_POST["fotos"] as $foto) { 
                $cons="insert into jcms_productos_imagenes (id_producto,imagen) values ('".$id."','".$foto."')";
                mysql_query($cons);
            }*/
    ...
    if( isset( $_GET['id'] ) ) {
        ...
        <script type="text/javascript">
            ...
            function loadFotosInicio() {
                ...
                $exIP=mysql_query($sqlIP); $numfotos=0;
                while( $rowIP=mysql_fetch_object($exIP) ) {                    
                    ?>$('#div_fotos').append('<div id="fotothumb<? echo $numfotos ?>" class="elementofoto" style="position:relative; float:left; width:70px; height:70px; margin-right:10px;"><img src="../thumbProducto.php?foto=images/<? echo $rowIP->imagen; ?>" /><input type="hidden" name="fotos[]" value="<? echo $rowIP->imagen; ?>" /><div style="width:20px; height:20px; position:absolute; top:0px; right:0px; z-index:10;"><a class="enlacequitar" href="javascript:quitarfoto(<? echo $numfotos; ?>);"><img src="images/borrar.png" /></a></div><div style="position:absolute; top:60px; left:17px; width:16px; height:16px; z-index:10;"><a class="enlacemenos" href="javascript:orden_menos(<? echo $numfotos; ?>);"><img src="images/izq.png" /></a></div><div style="position:absolute; top:61px; left:33px; width:16px; height:16px; z-index:10;"><a class="enlacemas" href="javascript:orden_mas(<? echo $numfotos; ?>);"><img src="images/der.png" /></a></div></div>');
                    var array_foto=new Array();
                    array_foto[0]='<? echo $rowIP->imagen; ?>';
                    array_foto[1]=<? echo $numfotos ?>;
            ...
            function quitarfoto(id) { //alert(id);
                $('#fotothumb'+id).remove();
                if( id==0 ) {
                    array_fotos.shift();
                } else {
            ...
            function orden_menos(id) {
                if((id-1)>=0) {
                    ...
                    array_fotos[id][1]=parseFloat(array_fotos[id-1][1])-(0.001); //alert(dump(array_fotos));
                    array_fotos.sort(function(a,b){
                        if(a[1] < b[1]) return -1;
                        if(a[1] > b[1]) return 1;
                        return 0;            
                    });
                    reordenar_fotos(); //alert(dump(array_fotos));
                }
            }
            ...
            function dump(arr,level) {
            	var dumped_text = "";
            	if(!level) level = 0; //The padding given at the beginning of the line.
            	var level_padding = "";
    	...
    	<div><input name="nombre" id="nombre" type="text" style="width: 500px;" value="<? echo $rowProducto->nombre; ?>" /></div>
        
    	<div>Product name <img src="images/en.png" /></div>
    	<div><input name="nombre_en" id="nombre_en" type="text" style="width: 500px;" value="<? echo $rowProducto->nombre_en; ?>" /></div>        
    	
        <div style="margin-top: 10px;">Orden</div>
    	<div><input name="orden" id="orden" type="text" value="<? echo $rowProducto->orden; ?>" style="width: 50px;" /></div>               
    	
        <div style="margin-top: 10px;">Precio</div>
    	<div><input name="precio" id="precio" type="text" style="width: 100px;" value="<? echo $rowProducto->precio; ?>" /></div>    
    	
        <div style="margin-top: 10px;">Descripción <img src="images/es.png" /></div>
    	<div><textarea name="descripcion" id="descripcion" style="width: 639px; height:400px;"><? echo $rowProducto->descripcion; ?></textarea></div>
        
        <div style="margin-top: 10px;">Description <img src="images/en.png" /></div>
    	<div><textarea name="descripcion_en" id="descripcion_en" style="width: 639px; height:400px;"><? echo $rowProducto->descripcion_en; ?></textarea></div>
        ...
        <div><select name="id_seccion" style="width: 300px;"><? 
            /*$sql1="select * from jcms_secciones order by orden asc";
            $ex1=mysql_query($sql1);
            while($row1=mysql_fetch_object($ex1)) {
                ?><option value="<? echo $row1->id; ?>" <? if( $row1->id==$rowProducto->id_seccion ) echo "selected"; ?>><? echo $row1->nombre; ?></option><?
            }*/
            ...
            function buscar_hijos_rec($id_cat) {
                ...
                if( mysql_num_rows(mysql_query("select * from jcms_secciones where id_padre='".$id_cat."'"))>0 ) {
                    ...
                    while( $rowC=mysql_fetch_object($exC) ) {            
                        $id=$rowC->id;
                        ?><option value="<? echo $id; ?>" <? if( $idSeccion==$id ) { echo "selected"; } ?>><? for($k=0;$k<($sub_level*3+3);$k++) echo "&nbsp;"; ?><? echo $rowC->nombre; ?></option><?
                        buscar_hijos_rec($id);
                    }        
                }
            }        
            ...
            while( $row2=mysql_fetch_object($ex2) ) {
                $id=$row2->id;
                ?><option value="<? echo $id; ?>" <? if( $idSeccion==$id ) { echo "selected"; } ?>><? for($k=0;$k<($sub_level*3+3);$k++) echo "&nbsp;"; ?><? echo $row2->nombre; ?></option><?
                buscar_hijos_rec($id);
                $sub_level=0;
            }               
