...
(function ($) {
	...
	function fileManagerObj(_handler) {
		...
		this.loadDir = function () {
			...
			$.getJSON(self.handler, { "dir": self.curDir, "action": "list", "timestamp": timest }, function (json) {
				if (json.success) {
 ...
 */
		...
		this.loadUploadUI = function () {
			...
			var self = this;
            //alert(self.upload.handler);
			var uiHtml = '<form enctype="multipart/form-data" method="post" action="' + self.upload.handler + '">' +
							'<p><div style="width:270px; overflow:hidden;"><input type="file" name="handle" /></div>' +
							'<input type="text" name="newName" style="width:250px; display:none; border:solid 1px !important;" />' +
							'<input type="text" name="action" style="display:none;" value="upload" /></p>' +
		...
		this.defaultTranslations = {
			...
			"upload_title":			"Subir fichero",
			"rename_title":			"Borrar fichero",
			"remove_title":			"Borrar titulo",
			"mkdir_title":			"Crear directorio",
			"upload_action": 		"Subir nuevo fichero al directorio actual",
			"mkdir_action": 		"Crear nuevo directorio",
			"remove_action": 		"Borrar este fichero",
			"rename_action": 		"Renombrar este fichero",
			"delete_message": 		"Seguro de borrar?",
			"new_directory": 		"Nuevo directorio",
			"previous_directory": 	"Ir al directorio anterior",
			"rename":				"Renombrar",
			"select": 				"Seleccionar",
			"create": 				"Crear",
			"submit": 				"Enviar",
