                ...
                if (isset($_GET['id'])) {
                    ...
                    <div><select name="id_padre" style="width: 300px;">
                        ...
                        function buscar_hijos_rec($id_cat) {
                            ...
                            if (mysql_num_rows(mysql_query("select * from jcms_secciones where id<>'" . $id_seccion . "' and id_padre='" . $id_cat . "'"))>0) {
                                ...
                                while ($rowC=mysql_fetch_object($exC)) {
                                    $id = $rowC->id;
                                    ?><option value="<?php echo $id; ?>" <?php if ($idPadre == $id) { echo "selected"; } ?>><?php for ($k=0; $k < ($sub_level * 3 + 3); $k++) echo "&nbsp;"; ?><?php echo $rowC->nombre; ?></option><?php
                                    buscar_hijos_rec($id);
                                }
                            }
                        }
                        ...
                        while ($row2 = mysql_fetch_object($ex2)) {
                            $id = $row2->id;
                            ?><option value="<?php echo $id; ?>" <?php if ($rowSeccion->id_padre == $id) { echo "selected"; } ?>><?php for ($k=0; $k<($sub_level * 3 + 3); $k++) echo "&nbsp;"; ?><?php echo $row2->nombre; ?></option><?php
                            buscar_hijos_rec($id);
                            $sub_level = 0;
                        }
